...
const MoveModal = ({ tree,visible,load,source_id,target_id,bus })=>{
	
	const onSelect = ( keys ) =>{
		let key = keys[0]
		if(key){
			bus.publish("MOVE_MODAL_CHANGE",{ key })
		}else{
			bus.publish("MOVE_MODAL_CHANGE",{ key:"" })
		}
	}
	...
	let onOk = ()=>{
		...
		bus.publish("MOVE_MODAL_CONFIRM",{source_id,target_id})
	}
	
	const onCancel = ()=>{
		bus.publish("MOVE_MODAL_CLOSE",{});
	}
